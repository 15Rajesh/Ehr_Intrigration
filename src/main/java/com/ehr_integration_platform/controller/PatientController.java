...
public class PatientController {
    ...
    // CREATE
    @PostMapping
    public ApiResponse<PatientDTO> createPatient(@Valid @RequestBody PatientDTO dto) {
        PatientDTO saved = service.savePatient(dto);
        return new ApiResponse<>("Patient created successfully", saved);
    }
    ...
    public ApiResponse<?> getAllPatients(
        ...
        Page<PatientDTO> result =
                service.getAllPatients(page, size, sortBy, direction, name, gender, disease);

        if (result.isEmpty()) {
            return new ApiResponse<>("No data found", result.getContent());
        }
    ...
    public ApiResponse<PatientDTO> getPatient(@PathVariable Long id) {
        ...
        return new ApiResponse<>("Success", patient);
    }

    // GET BY MOBILE
    @GetMapping("/mobile/{number}")
    public ApiResponse<PatientDTO> getByMobile(@PathVariable String number) {
    ...
    public ApiResponse<?> searchByName(@RequestParam String name) {
        ...
        if (list.isEmpty()) {
            return new ApiResponse<>("No data found", list);
        }
    ...
    public ApiResponse<PatientDTO> partialUpdate(
        ...
        PatientDTO updated = service.updatePartial(id, dto);
        return new ApiResponse<>("Patient updated successfully", updated);
    }
    ...
    public ApiResponse<PatientDTO> updatePatient(
        ...
        dto.setId(id);
        PatientDTO updated = service.savePatient(dto);
        return new ApiResponse<>("Patient updated successfully", updated);
    }
